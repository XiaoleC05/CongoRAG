...
### 这次端到端验证**没有**覆盖到的：工具步骤的重放

上面那次崩溃卡在一轮**模型生成**的中途，所以恢复走的是"重放历史 + 让它接着
生成"那条路；而 `pendingToolSteps` 的重放（真的重新调用一次工具）**没有被
这一步覆盖**——要让工具步骤停在半途，得让进程正好死在 `Invoke` 执行的那一瞬
间，而内置工具是毫秒级的，崩溃探针打不准那个窗口。

替代的验证在 `internal/agent/postgres_integration_test.go` 的
`TestPgRepo_EffectJudgementSurvivesJSONBRoundTrip`：它直接在真库上把那个
**最难发现的性质**钉住了——同一个效果被算两次，一次用原始字节（执行侧，
参数来自 Eino 的事件）、一次用 jsonb 读回来的字节（恢复侧）。见下面那一条。

### 一次真实的教训：判据在真库上是反的，而单测全绿

工具效果账本（`tool_effect_log`）的判据问的是"这一步的副作用发生过没有"，
而判据要比较参数。执行侧拿到的是 **Eino 给的原始字节**，恢复侧拿到的是
**从 `agent_run_steps.tool_args` 读回来的**——那一列是 **jsonb**，Postgres
会重写它：
...
直接哈希原始字节的话两侧必然不同，于是恢复时判据**永远返回 false**，那一步
被排进重放列表、工具被**第二次执行**，而因为键不同、唯一约束那道兜底也不响：
**全程无错误、无日志**。整套机制存在的理由就是防这个。

**它躲过了全套单测**，因为假 repo 里字节原样存取，两侧当然相等——这条缺陷的
形状是"写入用 A、判读用 B"，只有真库那条 jsonb 往返能把它露出来。修法是
`EffectKey` 先规范化（解析后重新序列化）再哈希，判据本身改成只按 step 问。

记住这条的形状：**同一个量在两个地方各算一次，而其中一处的输入被存储层
规范化过。** 遇到它的时候，单测那种"存取原样"的假实现一定抓不到。
