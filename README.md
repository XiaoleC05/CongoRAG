...
深色是默认外观（`index.html` 上写死 `class="dark"`，浅色是切换出来的）。
下面的图都是**跑起来截的**，不是示意图；截图里的数据来自本机的一个演示库。

| 引导页：接入模型服务（BYOK） | 对话：流式回答 + citation |
| --- | --- |
| ![引导页](docs/assets/onboarding.png) | ![对话页](docs/assets/conversation.png) |

| 知识库：文档状态、分块数与检索调试 | Agent：执行轨迹与工具调用 |
| --- | --- |
| ![知识库详情](docs/assets/knowledge-base.png) | ![Agent 执行轨迹](docs/assets/agent-run.png) |

两张图各有一处值得说清：

- **对话页那张是"生成到一半"截的**——正文末尾的光标、已经随流下发的来源角标
  `[1] [2] [3] [4]`、以及同一个按钮位置上已经变成"停止"。这三样都是这一版的
  交互（停止按钮见 #79，citation 随流见 §九）。
- **知识库详情下半部分**是这个项目**唯一能直接看见检索质量**的地方
  （`POST /knowledge-bases/{id}/search`）："为什么这个问题答错了"是没检索到、
  还是排序不对，在这里能当场看出来。
