...
- 判据：**流式进行中，同一个按钮位置要变成"停止"**（`ConversationPage.tsx`）——BYOK 产品里每一个 token 都在花用户自己的额度，"能随时后悔"是流式体验里最贵的一课。
- 判据：**状态的中文映射与配色只有一处**（`components/agent/runStatus.ts` 的 `RUN_STATUS_LABEL`）。它用 `Record<RunStatus, …>` 而不是 `Record<string, …>`——契约加第七态时直接编译不过，不会静默漏一档。
- 判据：**重试与重新生成都要换一个新的幂等键**，不能复用上一轮那个。复用会命中服务端已有的记录、把那一轮**已经记下来的终态**（失败时就是那条 error 帧）补发一遍——用户再看到同一个错误，等于什么都没做。前者是"这一次没成功、再来一次"，后者是"换一次采样"，两者都不是"同一次尝试的重传"。
- 判据：**重试 / 重新生成进行中，入口撤掉**（不是禁用），并在**用户刚点的那一行**显示"重试中…"，否则用户不知道点的是哪一条。
...
- **排序与过滤时要连带给出"筛完没有结果"的空态**——它和"这里本来就没有东西"不是同一个空态，文案与出口都不一样（前者给"清除筛选"，后者给"去创建 / 去上传"）。两个列表现成的例子：`DocumentListToolbar.tsx` + `KnowledgeBaseDetailPage.tsx` 的两种空态、`RunListToolbar.tsx` + `AgentDetailPage.tsx` 的同样一对。
- **筛选控件用一排 `aria-pressed` 开关，不用下拉**：取值少、一屏可见、一次点击；另有一条测试环境的理由——`shadcn Select` 在 jsdom 下打不开（`target.hasPointerCapture is not a function`），`DropdownMenu` 在一个测试文件里只能开一次。
