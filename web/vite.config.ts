...
export default defineConfig({
  ...
  server: {
    ...
    //
    // 交付期这几条前缀由 apps/api 的 spa.go 排除，两边要保持一致。
    //
    // 【为什么要带上 /readyz】前端不请求它，但 spa.go 的 apiPrefixes 里有它
    // ——两条清单是同一份约定的两半，一边漏了就会在开发期和交付期表现不同
    // （开发期 Vite 把 /readyz/ 当静态路径，交付期 spa.go 回 404 Problem）。
    proxy: {
      '/api': 'http://127.0.0.1:3210',
      '/healthz': 'http://127.0.0.1:3210',
      '/readyz': 'http://127.0.0.1:3210',
    },
  },
})
