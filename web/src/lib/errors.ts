...
const MESSAGES: Record<string, string> = {
  ...
  // 【恢复被拒绝的三档（v4.0 新加，见 ADR-007）】它们只在
  // POST /api/v1/runs/{runId}/resume 上出现，而且都是"平台拒绝替你做一个
  // 有风险的判断"，不是"服务坏了"。文案要给出**下一步动作**，因为它对用户
  // 的可见后果是"这条运行不会自己好，只能重新发起一次"。
  //
  // 三档的区别值得保留（分成三句而不是一句"不能恢复"）：
  //   - 版本不兼容 → 重新发起；
  //   - 副作用可能已生效 → 重新发起，而且提醒他去确认那一步到底做没做；
  //   - 工具不允许重放 → 重新发起，或者把那个工具改成幂等的。
  state_schema_version_mismatch:
    '这条运行的快照是旧版本写的，无法从断点恢复——请重新发起一次运行',
  tool_effect_already_applied:
    '这一步的副作用可能已经生效但结果没留下来，平台不会自动重放它——请确认那一步的结果，然后重新发起一次运行',
  replay_unsafe: '这一步用的工具不允许被自动重放——请重新发起一次运行',
}
